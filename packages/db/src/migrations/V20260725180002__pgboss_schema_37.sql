-- US-046: owner-applied pg-boss schema.
-- Frozen from pg-boss 12.26.3 getConstructionPlans("pgboss"), schema version 37.
-- Never regenerate this file in place; upgrades require a new append-only migration.
...
CREATE TABLE pgboss.worker_job_execution (
...
CREATE TABLE pgboss.worker_runtime_health (
  instance_id text PRIMARY KEY,
  status text NOT NULL CHECK (status IN ('healthy', 'unhealthy', 'stopped')),
  scheduler_ready boolean NOT NULL,
...
-- Runtime is deliberately migration-disabled and never owns or creates schema objects.
REVOKE ALL ON SCHEMA pgboss FROM PUBLIC;
REVOKE ALL ON SCHEMA pgboss FROM ledger_app;
GRANT USAGE ON SCHEMA pgboss TO ledger_app;

REVOKE ALL ON TYPE pgboss.job_state FROM PUBLIC;
REVOKE ALL ON TYPE pgboss.job_state FROM ledger_app;
GRANT USAGE ON TYPE pgboss.job_state TO ledger_app;

REVOKE ALL PRIVILEGES ON ALL TABLES IN SCHEMA pgboss FROM PUBLIC;
REVOKE ALL PRIVILEGES ON ALL TABLES IN SCHEMA pgboss FROM ledger_app;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE
  pgboss.job,
  pgboss.job_common
TO ledger_app;
GRANT SELECT, DELETE ON TABLE pgboss.job_dependency TO ledger_app;
GRANT SELECT, INSERT ON TABLE pgboss.queue TO ledger_app;
GRANT UPDATE (
  retry_limit,
  retry_delay,
  retry_backoff,
  ...
  singletons_active,
  ready_history
) ON TABLE pgboss.queue TO ledger_app;
GRANT SELECT, INSERT ON TABLE pgboss.schedule TO ledger_app;
GRANT UPDATE (cron, timezone, data, options, updated_on)
  ON TABLE pgboss.schedule TO ledger_app;
GRANT SELECT ON TABLE pgboss.version TO ledger_app;
GRANT UPDATE (cron_on, flow_on) ON TABLE pgboss.version TO ledger_app;
GRANT SELECT, INSERT, UPDATE ON TABLE
  pgboss.worker_job_execution,
  pgboss.worker_runtime_health
TO ledger_app;

-- PostgreSQL grants EXECUTE on new functions to PUBLIC by default. Remove it
-- from every pg-boss function, then opt the application into only the queue
-- declaration function used with partition=false.
REVOKE ALL PRIVILEGES ON ALL FUNCTIONS IN SCHEMA pgboss FROM PUBLIC;
REVOKE ALL PRIVILEGES ON ALL FUNCTIONS IN SCHEMA pgboss FROM ledger_app;
GRANT EXECUTE ON FUNCTION pgboss.create_queue(text, jsonb) TO ledger_app;
