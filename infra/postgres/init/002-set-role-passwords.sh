...
: "${LEDGER_RESTORE_ADMIN_PASSWORD:?LEDGER_RESTORE_ADMIN_PASSWORD is required}"
...
psql --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" --set ON_ERROR_STOP=1 \
  --set ledger_restore_admin_password="$LEDGER_RESTORE_ADMIN_PASSWORD" \
  --command "ALTER ROLE ledger_restore_admin PASSWORD :'ledger_restore_admin_password';"
