...
the Nous product design pipeline. It is a **single self-hosted app** (route handlers,
...
| Security patterns (Auth.js/Keycloak sessions, role gating, tenant scoping) | `docs/dev-guide/SECURITY.md` | Adding an auth'd route handler |
...
3. **Data via drizzle** — the shared `db` client (import from `@smp/db`). Every company-scoped query filters the table's `company_id` column. Reuse tables from `@smp/db/schema`; derive types with `$inferSelect`/`$inferInsert`.
4. **Auth via Auth.js and Keycloak** — `auth()` in route handlers (no session → 401); `useSession()` + `hasMinRole()` in the UI. Identity comes from the verified Keycloak OIDC session; roles and company grants come from Ledger DB, never the request.
...
| Tenant column | `company_id` (uuid) | every company-scoped table carries `company_id` |
