...
tpl.workspace-record. Tiles: purchased, assigned, pending, free. Tabs — Capacidad (VendorAccountCapacity history table + add row form), Credenciales (kind, last4, scopes, health, last_verified_at; rotate action), Tipos de licencia (LicenseType + RateCard current), Configuración (mode automated/orchestration toggle with explanation, low_pool_floor, renewal date). Data: VendorAccount(*), VendorAccountCapacity(*), IntegrationCredential(*), LicenseType(*), RateCard(*).
