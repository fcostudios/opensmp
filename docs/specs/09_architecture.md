...
**Inputs:** `04_er_model.md` (27 entities/8 contexts) · `08_scope.md` (58 story IDs, including the partitioned US-018 parent, 44 server actions) · PRD §13/§15 · DEC-SMP-001..013 · `extracts/_step8_coherence_pack.json`
...
| `vendor-catalog` | vendor_catalog_context | Vendor, VendorAccount, VendorAccountCapacity, LicenseType, IntegrationCredential, ConnectorCallObservation | createVendorAccount, updateVendorAccount, saveVendorAccountCapacity (canonical — ADR-06: registerPurchase/addCapacity are UI entry points), verifyCredential, rotateCredential |
...
`packages/connectors`: `capabilities()` + provision/deprovision/syncMembers/syncActivity/syncCost; dispatch by `Vendor.provisioning_protocol` (rest/scim/none); unsupported *provisioning* capability ⇒ orchestration checklist ProvisioningAction (DEC-SMP-007); unsupported *sync* capability (or `VendorAccount.ingestion_mode ∈ {csv_import, manual}`) ⇒ the API-less ingestion path (DEC-SMP-018, US-055): `importMembersCsv`/`importUsageCsv` parse Console exports into the SAME upserts/diffs the API sync writes (source provenance on ActivityRecord/CostRecord), and manual register upkeep runs under the same DB-level integrity + audit. An org on a Claude Teams plan (no Admin/Analytics API) is fully manageable on this path.

Anthropic connector #1 is delivered by the official US-018 partition: US-056 owns endpoint policy, strict Admin/Analytics credential routing, per-account rate limiters (100/min UM, 60/min analytics, 1 200 invites/h), and bounded operation-aware retry; US-057 owns the append-only connector-call journal and persistence port; US-058 owns the five neutral operations, dispatcher registration, and Pact-backed conformance. `anthropic-version` and endpoint-specific beta policy are pinned in one provider module. The connector package accepts injected credential, clock, sleep, transport, and observation ports and imports neither application modules nor Drizzle.

For every network attempt the composition layer commits a sanitized `ConnectorCallObservation(phase=requested)` before transport, then appends `succeeded` or `failed` when the result is known. Persistence failure before `fetch` prevents the call; interruption or ambiguous failure leaves the unmatched requested event visible. Provisioning observations may link to an existing request-owned `ProvisioningAction`; account-level sync observations do not fabricate a `LicenseRequest`. No raw provider body crosses the connector boundary or enters the journal. Core modules import the neutral interface only (lint guard).
...
| Auditability (figure → rows → governed evidence) | StatementLine.assignment_id chain + telemetry replay payloads; append-only ConnectorCallObservation sanitized summaries; AuditLog | US-033/036/057 |
