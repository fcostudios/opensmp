...
import { createVendorAccountSchema } from "@smp/contracts";
...
export function VendorAccountForm({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const parsed = createVendorAccountSchema.safeParse({
      contractRenewalOn: formData.get("contractRenewalOn"),
      lowPoolFloor: numberField(formData, "lowPoolFloor"),
      mode: formData.get("mode"),
      name: formData.get("name"),
      vendorId: formData.get("vendorId"),
      vendorOrgRef: formData.get("vendorOrgRef"),
    });
    if (!parsed.success) {
      setState({
        status: "error",
    ...
    >
      ...
      <div className="flex flex-wrap justify-end gap-2">
        ...
        <button
          ...
          data-testid="btn_create_vendor_account"
          disabled={pending}
          type="submit"
        >
